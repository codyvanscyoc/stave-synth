...
- 5" capacitive DSI touchscreen (800x480)
- Secondary target profile: Pi 4 (2GB) — headless, Fluid-only piano

## Architecture

- **PipeWire-JACK** for audio. App must run via `pw-jack` prefix — real jackd doesn't output audio on this Pi 5 Trixie setup.
- **C bridge** (`stave_synth/jack_bridge.c` → `jack_bridge.so`): handles JACK process callback natively because python-jack-client CFFI is broken on aarch64/Pi 5. Ring buffer (16 slots normal / 6 low-latency, runtime-switchable) lets Python render ahead.
...
High-level architecture: 24-voice Faust polysynth + piano (FluidSynth + 4 soundfont presets + 7 voicings) + B3 organ (Faust) + 7 reverb engines + 2 LFOs (poly + tempo-sync + shape lib) + ping-pong delay + sympathetic resonance + bus comp (SSL G) + per-OSC ADSR/filter/pan + macros + setlist + recorder + WebSocket UI. Tested live in worship every week. ~50% CPU at full load on Pi 5.
