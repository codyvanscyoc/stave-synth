...
class NativeAuditionSourceTests(unittest.TestCase):
    ...
    def test_driver_is_fail_closed_and_callback_has_no_forbidden_work(self):
        ...
        self.assertIn("midi_first_callback_monotonic_ns", source)
        self.assertIn("capture_first_callback_monotonic_ns", source)
        self.assertEqual(source.count("midi_first_callback_monotonic_ns"), 2)
        self.assertLess(source.index("if (!monotonic_ns())"), source.index("jack_activate(g_capture_client)"))
