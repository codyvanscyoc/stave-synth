...
class RenderMetricsIntegrationTests(unittest.TestCase):
    ...
    def test_record_covers_piano_synth_limiter_and_bridge_write(self):
        ...
        piano = source.index("instrument_player.render_block(bs)")
        self.assertIn("instrument_player = self.piano_player", source[timer:piano])
        self.assertIn("control_player.pump_render_controls()", source[timer:piano])
