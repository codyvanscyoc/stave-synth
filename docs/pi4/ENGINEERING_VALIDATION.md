...
The historical bodies of `test_2_param_flood.py`, `test_3_connection_chaos.py`, and `test_4_boundary_sweep.py` exercise live endpoints/routing and generic process discovery. They are now retired with unconditional direct-run refusal and discovery skip before dangerous imports. Do not remove those guards to qualify a release. The old boundary sweep persisted extreme settings with incomplete restoration; the old connection-chaos script reconnected JACK itself and did not prove automatic recovery. Use the reviewed offline allowlist and separately approved isolated hardware procedures.
