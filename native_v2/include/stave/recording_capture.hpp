...
namespace stave {
enum class CaptureEnd : unsigned { Open, Complete, Overflow, InvalidAudio, FrameLimit, WriterError, EngineStopped };
...
public:
    ...
    explicit RecordingCapture(unsigned frames=512,std::uint64_t limit=maximum_frames)
        :frames_(frames),limit_(limit) {
    ...
    bool push(const float* left,const float* right,unsigned frames) noexcept {
        ...
        if(written_>limit_-frames_) return finish(CaptureEnd::FrameLimit);
    ...
    bool finish(CaptureEnd reason=CaptureEnd::Complete) noexcept {
        if(reason!=CaptureEnd::Open&&end()==CaptureEnd::Open)
...
private:
    ...
    unsigned frames_;
    std::uint64_t limit_,written_{}; // audio-owner only
    std::array<Block,Capacity> blocks_{};
    alignas(64) std::atomic<std::uint64_t> head_{0},tail_{0};
    std::atomic<CaptureEnd> end_{CaptureEnd::Open};
