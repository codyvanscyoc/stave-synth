...
Global motion/filter drift now also run under StageInstrument ownership,
including key-sync, poly decisions and master sidechain. Four new joined
merged/nonmerged reference runs pass (fourteen full runs total); the earlier
overload gate still fails unchanged. See
[connected motion evidence](../docs/pi4/NATIVE_V2_MOTION_CONNECTED.md).
The standalone component runner requires NumPy/SciPy, no Faust/SF2, and retains
its 25,920-LFO/6,000-filter-block comparisons.
...
reverb replay demonstrates the overload input-rounding divergence. Owned motion
integration now passes both merged/nonmerged references. Complete recorded
beds/drone and remaining features, sound acceptance, controls and event adapter.
