"""Dattorro plate reverb (libstave_plate.so).
...
_LIB = _HERE / "libstave_plate.so"

_ffi = FFI()
_ffi.cdef("""
typedef struct StavePlate StavePlate;
...
StavePlate* newStavePlate(void);
void deleteStavePlate(StavePlate*);
void initStavePlate(StavePlate*, int sample_rate);
void instanceClearStavePlate(StavePlate*);
void buildUserInterfaceStavePlate(StavePlate*, UIGlue* ui);
void computeStavePlate(StavePlate*, int count, float** inputs, float** outputs);
""")
...
class FaustPlate:
    """Dattorro plate reverb — same I/O signature as FaustReverb."""

    def __init__(self, sample_rate: int = 48000):
        self.sample_rate = int(sample_rate)
        self._dsp = _lib.newStavePlate()
        if self._dsp == _ffi.NULL:
            raise RuntimeError("newStavePlate() returned NULL")
        _lib.initStavePlate(self._dsp, self.sample_rate)
    ...
    def __del__(self):
        try:
            if getattr(self, "_dsp", None):
                _lib.deleteStavePlate(self._dsp)
                self._dsp = None
        except Exception:
            pass

    def clear(self):
        _lib.instanceClearStavePlate(self._dsp)
    ...
    def process(self, samples: np.ndarray) -> np.ndarray:
        ...
        _lib.computeStavePlate(self._dsp, n, self._in_ptrs, self._out_ptrs)
        out = self._out_f64
...
def _install_ui_callbacks(dsp, zones: dict):
    ...
    _lib.buildUserInterfaceStavePlate(dsp, glue)
